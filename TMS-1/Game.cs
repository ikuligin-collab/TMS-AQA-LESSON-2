...
class Game
{
    ...
    public int RoundsToPlay
    {
        ...
        private set
        {
            ...
            {
                _roundsToPlay = 1; // Значение по умолчанию, если что-то пошло не так
            }
        }
    }

    // Конструктор принимает игроков и количество раундов
    ...
    public void Play()
    {
        ...
        while (_roundsPlayed < RoundsToPlay)
        {
            ...
            // Если ход невалидный, он не засчитывается, раунд не увеличивается
            ...
            // Обработка результатов раунда отдельным методом
            GameResult result = ProcessRound(playerMove, computerMove);

            // Вывод информации после каждого раунда
        ...
        // Итоги всей игры после завершения цикла
        PrintFinalResults();
    }
